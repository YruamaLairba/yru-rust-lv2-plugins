...
const PACKAGES_CONF: &[PackageConf] = &[
    PackageConf {
        name: "yru-echo-rs",
        post_build: |conf| {
            let built_bin_name = [&conf.lib_prefix(), "yru_echo_rs", &conf.lib_suffix()].concat();
            let lib_file_name = [&conf.lib_prefix(), "yru-echo-rs", &conf.lib_suffix()].concat();
            let subs: &[(&str, &str)] = &[("@LIB_FILE_NAME@", &lib_file_name)];
            let src_dir = workspace_root().join("yru-echo-rs");
            let out_dir = conf.build_dir().join("lv2").join("yru-echo-rs");
    ...
    PackageConf {
        ...
        post_build: |conf| {
            let built_bin_name = [&conf.lib_prefix(), "yru_chorus_rs_stereo", &conf.lib_suffix()].concat();
            let lib_file_name = [&conf.lib_prefix(), "yru-chorus-rs-stereo", &conf.lib_suffix()].concat();
            let subs: &[(&str, &str)] = &[("@LIB_FILE_NAME@", &lib_file_name)];
    ...
    PackageConf {
        name: "yru-flanger-rs",
        post_build: |conf| {
            let built_bin_name = [&conf.lib_prefix(), "yru_flanger_rs", &conf.lib_suffix()].concat();
            let lib_file_name = [&conf.lib_prefix(), "yru-flanger-rs", &conf.lib_suffix()].concat();
            let subs: &[(&str, &str)] = &[("@LIB_FILE_NAME@", &lib_file_name)];
            let src_dir = workspace_root().join("yru-flanger-rs");
            let out_dir = conf.build_dir().join("lv2").join("yru-flanger-rs");
            fs::create_dir_all(&out_dir).unwrap();
            subst(
                src_dir.join("manifest.ttl"),
                out_dir.join("manifest.ttl"),
    ...
    PackageConf {
        name: "yru-flanger-stereo-rs",
        post_build: |conf| {
            let built_bin_name = [&conf.lib_prefix(), "yru_flanger_stereo_rs", &conf.lib_suffix()].concat();
            let lib_file_name = [&conf.lib_prefix(), "yru-flanger-stereo-rs", &conf.lib_suffix()].concat();
            let subs: &[(&str, &str)] = &[("@LIB_FILE_NAME@", &lib_file_name)];
            let src_dir = workspace_root().join("yru-flanger-stereo-rs");
            let out_dir = conf.build_dir().join("lv2").join("yru-flanger-stereo-rs");
            fs::create_dir_all(&out_dir).unwrap();
            subst(
                src_dir.join("manifest.ttl"),
                out_dir.join("manifest.ttl"),
            ...
            .unwrap();
            for e in &["yru-flanger-stereo-rs.ttl"] {
                fs::copy(src_dir.join(e), out_dir.join(e)).unwrap();
            }
            fs::copy(
                conf.build_dir().join(&built_bin_name),
